...
.album_status { float: left; margin: 0!important; height:auto;  padding: 0;  text-align: right;  width: 100%;}
  ...
  <?php if(Auth::user()->contribute == 'contribute'){ ?>
	<div class="admin-section-title">
		<div class="row">
			<div class="col-md-8">
				<h3><i class="entypo-video"></i>Contributor Videos</h3><!--<a href="{{ URL::to('admin/videos/create') }}" class="btn btn-success"><i class="fa fa-plus-circle"></i> Add New</a>-->
			</div>
			<div class="col-md-4">
				<form method="get" role="form" class="search-form-full">
					<div class="form-group" >
						<select class="form-control" name="status">
							<option value="0" {{ (request('published') == 0) ? 'selected' : ''}}>All</option>
							<option value="1" {{ (request('published') == 1) ? 'selected' : ''}}>Published</option>
							<option value="2" {{ (request('published') == 2) ? 'selected' : ''}}>Unpublished</option>
						</select>
    ...
    <div class="col-sm-6 col-md-3">
		<article class="album">
			<header>
				...
				<a href="{{ URL::to('video/') . '/' . $video->id . '/' . $string }}" target="_blank">
