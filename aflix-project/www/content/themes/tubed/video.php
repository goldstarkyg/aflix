...
<div class="col-md-8 col-md-offset-2 right-content-8" id="video_section" style="padding:0px;">
	...
	<div id="video_bg" style="background-image:url(<?= Config::get('site.uploads_url') . '/images/' . str_replace(' ', '%20', $video->image) ?>)">
					 ...
					 //start** if video access is guest
               		 if( $video->access == 'guest' ){
							...
							if($video->type == 'embed'){
								if(! is_null($ad)){
			?>
									<button class= "ad_counter" style="position: absolute; z-index: 2147483647; bottom: 56px; left: 39px; background: none repeat scroll 0% 0% rgba(246, 103, 35, 0.70); color: rgb(255, 255, 255); padding: 9px 20px; border:0px;">Advertisement</button>
									<button id= "skip" style="display:none;position: absolute; z-index: 2147483647; bottom: 56px; right: 39px; background: none repeat scroll 0% 0% rgba(16, 173, 60, 0.70); color: rgb(255, 255, 255); padding: 9px 20px; border:0px;"> Skip</button>
									<style>
		...
		?>
						...
						<a href="<?php echo $video->ads_link;?>" target="_blank"><div class="video_link_placeholder"></div></a>
					<?php
					}
					?>
					<div id="video_container" class="fitvid">
	...
	<?php
		@$roll=Auth::user()->role;
		if($video->live=="live" and @$p!=null AND @$roll=="subscriber" OR (@$roll=="admin" AND $video->live=="live")){
	?>
	<!-- bootstrap -->
